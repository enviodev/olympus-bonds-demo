...
import { 
  getTokenDecimals, 
  getFixedExpiryMarketData, 
  getFixedExpiryMarketMetadata 
...
BondFixedExpirySDA.MarketCreated.handler(async ({ event, context }) => {
  ...
  try {
    const marketId = getId(event.srcAddress, BOND_TYPE, event.params.id);
            ...
            const [marketData, marketMetadata, payoutTokenDecimals, quoteTokenDecimals] = await Promise.all([
              context.effect(getFixedExpiryMarketData, {
                contractAddress: event.srcAddress,
                marketId: event.params.id.toString(),
                blockNumber: event.block.number,
              }),
              context.effect(getFixedExpiryMarketMetadata, {
                contractAddress: event.srcAddress,
                marketId: event.params.id.toString(),
                blockNumber: event.block.number,
              }),
...
BondFixedExpirySDA.MarketClosed.handler(async ({ event, context }) => {
  const marketId = getId(event.srcAddress, BOND_TYPE, event.params.id);
