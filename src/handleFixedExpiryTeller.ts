...
import { 
  getTokenDecimals, 
  getAuctioneer, 
  getFixedExpiryMarketInfoForPurchase 
...
BondFixedExpiryTeller.Bonded.handler(async ({ event, context }) => {
  ...
  try {
    ...
    const marketInfo = await context.effect(getFixedExpiryMarketInfoForPurchase, {
